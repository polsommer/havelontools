...
public class player_money extends script.base_script
{
    ...
    public int handleDepositWithdraw(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        {
            case sui.BP_CANCEL:
                removeObjVar(self,XOBJ);
                removeObjVar(self,YOBJ);
                removeObjVar(self,ZOBJ);
                removeObjVar(self,SOBJ);
            return SCRIPT_CONTINUE;
        }
        float distance = 0;
        if (hasObjVar(self,XOBJ)){
            distance = getDistance(self,
                    new location(
                            getFloatObjVar(self, XOBJ),
                            getFloatObjVar(self, YOBJ),
                            getFloatObjVar(self, ZOBJ),
                            getStringObjVar(self,SOBJ)));
        }
        if (distance > 3){
            sendSystemMessageTestingOnly(self,"You are too far from the bank. Please move closer and try again.");
            removeObjVar(self,XOBJ);
            removeObjVar(self,YOBJ);
            removeObjVar(self,ZOBJ);
            removeObjVar(self,SOBJ);
