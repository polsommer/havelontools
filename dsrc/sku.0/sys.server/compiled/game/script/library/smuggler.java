...
public class smuggler extends script.base_script
{
    ...
    {
        sendSystemMessageTestingOnly(player, "WARNING: Buyback is currently disabled. Please confirm all transactions with the junk dealer.");
    ...
    public static void sellJunkItem(obj_id player, obj_id item, boolean fence, boolean reshowSui) throws InterruptedException
    {
        ...
        if (price > 0 && !hasObjVar(item, "quest_item"))
        {
            ...
            //money.systemPayout(money.ACCT_JUNK_DEALER, player, price, "handleSoldJunk", params); // place this line back to restore function.
            // @todo this is a temporary solution to the credit duplication glitch.
            sendSystemMessageTestingOnly(player, "You have sold "+getName(item)+" for "+price+" credits. Buyback is currently disabled.");
            destroyObject(item);
            money.systemPayout(money.ACCT_JUNK_DEALER, player, price, money.DICT_PAY_HANDLER, null);
            // @end
        }
        else 
        {
