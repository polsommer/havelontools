...
public class resource extends script.base_script
{
    ...
    public static float HAND_SAMPLE_BONUS = 2;
    ...
    public static int getSample(obj_id user, obj_id tool, String type) throws InterruptedException
    {
        ...
        if (density > threshold)
        {
            ...
            if ((roll <= chance) || (gamble > 0) || (nodecritloc != null))
            {
                ...
                float HAND_SAMPLE_BONUS = utils.stringToFloat(getConfigSetting("GameServer", "harvesterExtractionRateMultiplier"));
                if (HAND_SAMPLE_BONUS >1){
                    amt = (int) (amt * HAND_SAMPLE_BONUS);
                    if (hasObjVar(user,"sampling.test")){
                        amt = amt *2;
                    }
                }
