...
public class player_structure extends script.base_script
{
    ...
    public static boolean canPlaceStructure(obj_id player, String template, location loc, obj_id deed) throws InterruptedException
    {
        ...
        if (!isGameObjectTypeOf(getGameObjectType(deed), GOT_data_house_control_device))
        {
            ...
            {
                if (isGod(player)){sendSystemMessageTestingOnly(player,"You cant own the structure so we are returning false");}
                return false;
            }
        }
        ...
        if (canOwnStructure(template,player,false)){
            if (isGod(player)){sendSystemMessageTestingOnly(player,"You can place the structure.");}
            return true;
        }
        if (isGod(player)){sendSystemMessageTestingOnly(player,"You can not place the structure.");}
        return false;
    }
    ...
    public static boolean canOwnStructure(String template, obj_id player, boolean allowOverLotLimitIfPlayerHouse) throws InterruptedException
    {
        ...
        if (dataTableGetInt(PLAYER_STRUCTURE_DATATABLE, idx, DATATABLE_COL_NO_LOT_REQ) != 1)
        {
            ...
            if (isGod(player)){sendSystemMessageTestingOnly(player,"Your account lots are: "+lots);}
            int lots_needed = (getNumberOfLots(template) / 4) - dataTableGetInt(PLAYER_STRUCTURE_DATATABLE, idx, DATATABLE_COL_LOT_REDUCTION);
            if (isGod(player)){sendSystemMessageTestingOnly(player,"You need this many lots: "+lots_needed);}
            if (lots_needed < 1)
            {
                lots_needed = 1;
            }
            ...
            if (isGod(player)){sendSystemMessageTestingOnly(player,"Do we care about player houses? "+allowOverLotLimitIfPlayerHouse);}
            if ((lots + lots_needed) > MAX_LOTS)
            {
                if (!allowOverLotLimitIfPlayerHouse || (template.indexOf("object/building/player/player_house_") != 0)) {
                    if (isGod(player)){sendSystemMessageTestingOnly(player,"You have exceeded the lot count and bounced you.");}
                    sendSystemMessageProse(player, prose.getPackage(new string_id(STF_FILE, "not_enough_lots"), lots_needed));
                    return false;
                }
            }
