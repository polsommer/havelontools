    ...
    public int handleCreateChosenResourceConfirm(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        switch (bp)
        {
            ...
            
            {
                ...
                if (isIdValid(resourceChosen))
                {
                    String COUNT_OBJVAR = "veteran_rewards.resource_redemption";
                    obj_id player = sui.getPlayerId(params);
                    if (getIntObjVar(player,COUNT_OBJVAR)>=2){
                        sendSystemMessageTestingOnly(player,"Sorry, you have already claimed your resources and may not claim any more on this account.");
                        return SCRIPT_CONTINUE;
                    }
                    obj_id crate = createResourceCrate(resourceChosen, RESOURCE_AMOUNT, utils.getInventoryContainer(player));
                    if (isIdValid(crate))
                    {
                        if (hasObjVar(player,COUNT_OBJVAR)){
                            int amt = getIntObjVar(player, COUNT_OBJVAR);
                            amt++;
                            setObjVar(player,COUNT_OBJVAR,amt);
                        }else{
                            setObjVar(player, COUNT_OBJVAR, 1);
                        }
                        setObjVar(self, OBJVAR_RESOURCE_REWARDED, true);
                        messageTo(self, "handleVeteranHarvestDestroy", null, 1, false);
                        sendSystemMessageGalaxyTestingOnly(getPlayerName(player)+" has redeemed a resource crate and received "+ RESOURCE_AMOUNT + " of resource type " +getResourceName(resourceChosen)+". They have claimed "+getIntObjVar(player, COUNT_OBJVAR)+" crate(s).");
