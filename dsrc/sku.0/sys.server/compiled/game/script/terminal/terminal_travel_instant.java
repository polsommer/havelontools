...
public class terminal_travel_instant extends script.base_script
{
    ...
    public static final String TRAVEL_COOLDOWN_OBJVAR = "travel_cooldown";
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        if (item == menu_info_types.ITEM_USE || item == menu_info_types.ITEM_USE_OTHER) {
            ...
            if (hasObjVar(player, TRAVEL_COOLDOWN_OBJVAR))
            {
                int time = getIntObjVar(player, TRAVEL_COOLDOWN_OBJVAR);
                int time_dif = (getGameTime()) - time;
                if (time_dif<1300){

                    int minutes = time_dif/60;
                    sendSystemMessageTestingOnly(player, "The Vehicle needs to charge and refuel before you use it again. You estimate it will be ready to use in "+(30-minutes)+" minutes.");
                    return SCRIPT_CONTINUE;
                }
            }
            ...
            if (success) {
                ...
                setObjVar(player,TRAVEL_COOLDOWN_OBJVAR,getGameTime());
            }
            return SCRIPT_CONTINUE;
        }
        return SCRIPT_CONTINUE;
