...
public class bank extends script.terminal.base.base_terminal
{
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        ...
        if (item == menu_info_types.ITEM_USE)
        {
            ...
            String YOBJ = "bank.y";
            String ZOBJ = "bank.z";
            String SOBJ = "bank.scene";
            setObjVar(player,XOBJ,getLocation(self).x);
            setObjVar(player,YOBJ,getLocation(self).y);
            setObjVar(player,ZOBJ,getLocation(self).z);
            setObjVar(player,SOBJ,getLocation(self).area);
            openBankMenu(player);
        }
        ...
        else if (item == menu_info_types.SERVER_MENU3)
        {
            ...
            if (getDistance(player,getLocation(bankTerminal))>1.25f){
                sendSystemMessageTestingOnly(player,"You are to far from the terminal. The transaction has been terminated.");
                return SCRIPT_CONTINUE;
            }
            if (amt == 0)
            {
